...
export function HookPage({ hook }: { hook: HookEntry }) {
  ...
  const callIndex = usageLines.findIndex((line) =>
    ...
    <div className="flex gap-10">
      <article className="min-w-0 max-w-3xl flex-1">
        ...
        <p className="mt-4 leading-relaxed text-gray-body">
          {linkifyWebApis(hook.description)}
        </p>
          ...
          <>
            ...
            {doc.typeAliases?.map((alias) => (
              <div key={alias.name} className="mt-8">
                <h3 className="scroll-mt-24 font-mono text-lg text-fg">
                  {alias.name}
                </h3>
                {alias.description && (
        ...
        {source && (
          <>
            ...
            <p className="mt-4 text-sm text-gray-body">
              The implementation, straight from hookli. Copy it, or install the
              package.
            </p>
            <CodeBlock
              code={source.source}
        ...
        <nav
          ...
          className="mt-12 flex items-center justify-between gap-4 border-t border-slate-syntax/40 pt-6"
        >
          {prev ? (
            <Link
              href={`/docs/${prev.slug}`}
      ...
      <aside className="hidden w-56 shrink-0 xl:block">
        <div className="sticky top-24 flex flex-col gap-8">
          ...
          <div className="rounded-xl border border-slate-syntax/40 bg-ground-raised p-4">
            ...
            <p className="mt-1 text-xs leading-relaxed text-gray-body">
              Free and open source. Star it or help it grow.
            </p>
            <Link
              href="/support"
              className="mt-3 inline-flex min-h-11 items-center gap-1 text-sm text-accent underline-offset-4 hover:underline"
            >
              Ways to support
              <ArrowRightIcon className="size-4" aria-hidden="true" />
            </Link>
